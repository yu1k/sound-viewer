...
// 各種コマンド
/****************************************/
// SwitchAudioSourceコマンド, brewでインストールしたコマンドなのでフルパスで指定したい
// intel mac
...
function getCurrentSoundOutputSourceInfo() {
    // execSyncの実行結果を格納 && returnする内容を格納する変数
    let showSound = null;
    ...
    try {
         ...
         */
        if (!((process.platform === "darwin" && process.arch === "x64") || (process.platform === "darwin" && process.arch === "arm64"))) {
        ...
        if (process.platform === "darwin" && process.arch === "x64") {
            showSound = execSync(`${iSwitchAudioSourceCommand} -c -f json`);
        }
        // apple silicon mac
        if (process.platform === "darwin" && process.arch === "arm64") {
            showSound = execSync(`${appleSiliconSwitchAudioSourceCommand} -c -f json`);
        }

        showSound = JSON.parse(showSound).name;
        return showSound;
    } catch (e) {
 ...
 * 表示するメニューはマシンに接続されているサウンドデバイスの数に応じて動的に生成する。
 * 現在選択されているサウンドデバイスまたは、この関数で選択したサウンドデバイスのメニューに checked: true でチェックを入れる。
 */
let boolFlag = null;
function selectSoundOutputDevicesFromContextMenuItem(){
    ...
    for (let i = 0; i < getAllAudioSourceDevicesInfoToObj.length; i++) {
        if(!((JSON.parse(getAllAudioSourceDevicesInfoToObj[i]).name) === getCurrentSoundOutputSourceInfo())){
            boolFlag = false;
        }
        if(((JSON.parse(getAllAudioSourceDevicesInfoToObj[i]).name) === getCurrentSoundOutputSourceInfo())){
            boolFlag = true;
